...
import animations.AnimMissileInvader;
import animations.Animable;

public class InvaderFire{
  ...
  public InvaderFire(float x, float y) {
    float size = 1f;// 0.7f;
    float width = 30 * size;
    float height = 12 * size;
    speed = 0.1f;
    state = States.MISSILE;

    fire = new Rectangle(x, y, width, height);
    anims = new Animable[3];
    anims[States.MISSILE.ordinal()] = new AnimMissileInvader(width, height, size);
  }
